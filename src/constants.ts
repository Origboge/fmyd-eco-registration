...
import type { StateLGAs } from "./types";

// State and LGA Data
export const STATE_LGAS: StateLGAs = {
    ...
    "Borno": ["Abadam", "Askira/Uba", "Bama", "Bayo", "Biu", "Chibok", "Damboa", "Dikwa", "Gubio", "Guzamala", "Gwoza", "Hawul", "Jere", "Kaga", "Kala/Balge", "Konduga", "Kukawa", "Kwaya Kusar", "Mafa", "Magumeri", "Maiduguri", "Maiduguri", "Marte", "Mobbar", "Monguno", "Ngala", "Nganzai", "Shani"],
...
// Image Helpers
// NOTE: These now use specific logic so you can replace them with your own file paths.

export const getLogo = () => {
    return "images/logoo.webp";
    // return "https://picsum.photos/150/150?random=logo";
}

export const getHeroBg = () => {
    return "images/waste2.webp";
    // return "https://picsum.photos/1920/1080?random=hero-main";
}

export const getAboutBg = () => {
    // Background for the 'About' section
    return "images/waste2.webp";

    
    // return "https://picsum.photos/1920/800?random=about-bg";
 }

// IMPLEMENTATION: Different Slides
export const getSlideImage = (index: number) => {
    // Define your specific images here
    const slides = [
        // "images/sdg-releve.webp", // Index 0 (Can be used for background)
        "images/purpose.webp",
        "images/sdg-releve.webp", // Index 1
        
    ];
    // Return the specific slide or a default
    return slides[index] || slides[0];
 }

// IMPLEMENTATION: Different Leaders
export const getLeaderImage = (name: string) => {
    const lowerName = name.toLowerCase();
    
    // Match "BOLA AHMED TINUBU GCFR"
    if (lowerName.includes("tinubu")) {
        return "/images/bola.webp";
    }
    
    // Match "COMR. AYODELE OLAWANDE"
    if (lowerName.includes("ayodele")) {
        return "/images/ayodele.webp";
    }
    ...
    // Fallback if no name matches
    const seed = name.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
    return `https://picsum.photos/300/300?random=leader-${seed}`;
}

// IMPLEMENTATION: Different Partners
export const getPartnerImage = (index: number) => {
    // Define your partner logos here
    const partners = [
    ...
    return partners[index - 1] || partners[0];
}
// Add this new exported function near your getPartnerImage function

export const getPartnerUrl = (index: number) => {
    // Define the partner URLs in the EXACT SAME order as the partner images in getPartnerImage()
    const partnerUrls = [
        "https://linktr.ee/SAMUapp", // URL for images/partner-1.webp (Index 1)
        "", // URL for images/partner-3.webp (Index 2)
        "",              // URL for images/doltma.webp (Index 3 - Assuming this is the DOLTMA link)
        "", // URL for images/partner-2.webp (Index 4)
        "", // URL for images/partner-4.webp (Index 5)
    ];
    
    // Returns the specific URL or an empty string ""
    return partnerUrls[index - 1] || "";
};

// IMPLEMENTATION: Different Activities
export const getTeamActivityImage = (index: number) => {
    // Define specific activity photos
    const activities = [
