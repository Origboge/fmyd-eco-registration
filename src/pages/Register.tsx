...
const Register: React.FC = () => {
    ...
    const handleSendOtp = async () => {
        ...
        // 2. Loop through and catch empty fields
        ...
        // 3. Check Specific Logic
        if (age !== null && age < 18) {
            newErrors['dob'] = 'You must be 18+ to register';
        }
        ...
        // Check Files
        if (!formData.passport) newErrors['passport'] = 'Passport photo is required';
        if (!formData.nin) newErrors['nin'] = 'NIN slip is required';
    ...
    const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (files && files[0]) {
            ...
            if (file.size > 25 * 1024) { 
                setValidationErrors(prev => ({...prev, [name]: "File too large. Max 25KB."}));
                e.target.value = ''; 
                setPreviews(prev => {
                ...
                <form className="p-6 md:p-10 space-y-8">
                    ...
                    <div className="animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
                        ...
                        <div className="grid md:grid-cols-3 gap-5">
                            <div>
                                <input required name="firstName" value={formData.firstName} onChange={handleChange} placeholder="First Name" className={getInputClass('firstName')} />
                                {validationErrors.firstName && <p className="text-red-500 text-xs mt-1 ml-1 font-medium flex items-center gap-1"><AlertCircle size={10} /> {validationErrors.firstName}</p>}
                            </div>
                            <div>
                                <input required name="middleName" value={formData.middleName} onChange={handleChange} placeholder="Middle Name" className={getInputClass('middleName')} />
                                {validationErrors.middleName && <p className="text-red-500 text-xs mt-1 ml-1 font-medium flex items-center gap-1"><AlertCircle size={10} /> {validationErrors.middleName}</p>}
                            </div>
                            <div>
                                <input required name="lastName" value={formData.lastName} onChange={handleChange} placeholder="Last Name" className={getInputClass('lastName')} />
                                {validationErrors.lastName && <p className="text-red-500 text-xs mt-1 ml-1 font-medium flex items-center gap-1"><AlertCircle size={10} /> {validationErrors.lastName}</p>}
                            </div>
                        </div>
                        <div className="grid md:grid-cols-2 gap-5 mt-5">
                            ...
                            <div>
                                <input required type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="Phone Number" className={getInputClass('phone')} />
                                {validationErrors.phone && <p className="text-red-500 text-xs mt-1 ml-1 font-medium flex items-center gap-1"><AlertCircle size={10} /> {validationErrors.phone}</p>}
                            </div>
                        </div>
                        <div className="grid md:grid-cols-2 gap-5 mt-5">
                            <div className="flex flex-col">
                                <label className="text-xs font-bold text-gray-500 mb-1 uppercase ml-1">Date of Birth</label>
                                <input required type="date" name="dob" value={formData.dob} onChange={handleChange} className={getInputClass('dob')} />
                                {validationErrors.dob && <p className="text-red-500 text-xs mt-1 ml-1 font-medium flex items-center gap-1"><AlertCircle size={10} /> {validationErrors.dob}</p>}
                            ...
                            <div className="flex flex-col">
                                ...
                                <select required name="sex" value={formData.sex} onChange={handleChange} className={getInputClass('sex')}>
                                    <option value="">Select Sex</option>
                                    <option value="Male">Male</option>
                                    <option value="Female">Female</option>
                                </select>
                                {validationErrors.sex && <p className="text-red-500 text-xs mt-1 ml-1 font-medium flex items-center gap-1"><AlertCircle size={10} /> {validationErrors.sex}</p>}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-1">
                        ...
                        <select id="stateOfOrigin" name="stateOfOrigin" value={formData.stateOfOrigin} onChange={handleChange} required className={getInputClass('stateOfOrigin')}>
                            <option value="" disabled>Select State of Origin</option>
                            {Object.keys(STATE_LGAS).map(stateName => (
                                <option key={stateName} value={stateName}>{stateName}</option>
                            ))}
                        </select>
                        {validationErrors.stateOfOrigin && <p className="text-red-500 text-xs mt-1 ml-1 font-medium flex items-center gap-1"><AlertCircle size={10} /> {validationErrors.stateOfOrigin}</p>}
                    </div>
                    ...
                    <div className="animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                        ...
                        <div className="grid md:grid-cols-2 gap-5">
                            <div className="flex flex-col">
                                <select required name="state" value={formData.state} onChange={handleChange} className={getInputClass('state')}>
                                    <option value="">Select State Of Residence</option>
                                    {Object.keys(STATE_LGAS).map(s => <option key={s} value={s}>{s}</option>)}
                                </select>
                                {validationErrors.state && <p className="text-red-500 text-xs mt-1 ml-1 font-medium flex items-center gap-1"><AlertCircle size={10} /> {validationErrors.state}</p>}
                            </div>
                            <div className="flex flex-col">
                                <select required name="lga" value={formData.lga} onChange={handleChange} disabled={!formData.state} className={`${getInputClass('lga')} disabled:bg-gray-100 disabled:text-gray-400`}>
                                    <option value="">Select LGA Of Residence</option>
                                    {lgas.map(l => <option key={l} value={l}>{l}</option>)}
                                </select>
                                {validationErrors.lga && <p className="text-red-500 text-xs mt-1 ml-1 font-medium flex items-center gap-1"><AlertCircle size={10} /> {validationErrors.lga}</p>}
                            </div>
                        </div>
                        <div className="mt-5">
                            <textarea required name="address" value={formData.address} onChange={handleChange} placeholder="Permanent Home Address" className={`h-24 resize-none ${getInputClass('address')}`} />
                            {validationErrors.address && <p className="text-red-500 text-xs mt-1 ml-1 font-medium flex items-center gap-1"><AlertCircle size={10} /> {validationErrors.address}</p>}
                        </div>
                        <div className="mt-5">
                            <textarea required name="landmark" value={formData.landmark} onChange={handleChange} placeholder="Nearest Landmark" className={`h-16 resize-none ${getInputClass('landmark')}`} />
                            {validationErrors.landmark && <p className="text-red-500 text-xs mt-1 ml-1 font-medium flex items-center gap-1"><AlertCircle size={10} /> {validationErrors.landmark}</p>}
                        </div>
                    </div>
                    ...
                    <div className="animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            {/* Passport */}
                            <div className="flex flex-col">
                                <div className={`relative border-2 border-dashed rounded-2xl p-2 h-64 flex flex-col items-center justify-center text-center transition-all group overflow-hidden ${validationErrors.passport ? 'border-red-500 bg-red-50' : (previews.passport ? 'border-brand-primary bg-brand-light/20' : 'border-gray-300 hover:border-brand-primary hover:bg-brand-light/10')}`}>
                                    {previews.passport ? (
                                        <>
                                        ...
                                        <div className="z-10 flex flex-col items-center p-6">
                                            <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 transition-colors ${validationErrors.passport ? 'bg-red-100 text-red-500' : 'bg-gray-100 text-gray-400 group-hover:bg-brand-primary group-hover:text-white'}`}>
                                                {validationErrors.passport ? <XCircle size={28} /> : <ImageIcon size={28} />}
                                            </div>
                                            <p className={`${validationErrors.passport ? 'text-red-600' : 'text-gray-700'} font-bold text-lg`}>Passport Photo</p>
                                ...
                                {validationErrors.passport && <p className="text-red-500 text-xs font-bold mt-2 ml-1 text-center">{validationErrors.passport}</p>}
                            </div>

                            {/* NIN */}
                            <div className="flex flex-col">
                                <div className={`relative border-2 border-dashed rounded-2xl p-2 h-64 flex flex-col items-center justify-center text-center transition-all group overflow-hidden ${validationErrors.nin ? 'border-red-500 bg-red-50' : (previews.nin ? 'border-brand-primary bg-brand-light/20' : 'border-gray-300 hover:border-brand-primary hover:bg-brand-light/10')}`}>
                                    {previews.nin ? (
                                        <>
                                        ...
                                        <div className="z-10 flex flex-col items-center p-6">
                                             <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 transition-colors ${validationErrors.nin ? 'bg-red-100 text-red-500' : 'bg-gray-100 text-gray-400 group-hover:bg-brand-primary group-hover:text-white'}`}>
                                                {validationErrors.nin ? <XCircle size={28} /> : <ImageIcon size={28} />}
                                            </div>
                                            <p className={`${validationErrors.nin ? 'text-red-600' : 'text-gray-700'} font-bold text-lg`}>NIN Slip</p>
                                            <p className="text-xs text-gray-500 mt-1">Click to browse (Max 25KB)</p>
                                        </div>
                                    )}
                                ...
                                {validationErrors.nin && <p className="text-red-500 text-xs font-bold mt-2 ml-1 text-center">{validationErrors.nin}</p>}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
                        ...
                        <select required name="trainingArea" value={formData.trainingArea} onChange={handleChange} className={`w-full text-lg py-3 ${getInputClass('trainingArea')}`}>
